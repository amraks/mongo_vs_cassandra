...
public class METSParser {
	...
	enum ParsePhaseEnum {
		OCR_FILEGRP, FILE, FLOCAT, STRUCTMAP, VOLUME_DIV, PAGE_DIV, OCR_FPTR;
	}

	public enum CopyrightEnum {
		PUBLIC_DOMAIN, IN_COPYRIGHT;
	}
	...
	public static class VolumeRecord {
			...
			public void addFeature(String feature) {
				if (features == null) {
					features = new ArrayList<String>(
							INITIAL_PAGE_FEATURE_LIST_SIZE);
				}
		...
		public VolumeRecord(String volumeID) {
			...
			this.pageCount = 0;
			this.hashMapByFilename = new HashMap<String, PageRecord>(
					INITIAL_HASHMAP_CAPACITY);
			this.hashMapByID = new HashMap<String, PageRecord>(
					INITIAL_HASHMAP_CAPACITY);
		...
		public void updatePageCount() {
			...
			pageCount = hashMapByID.size() > pageCount ? hashMapByID.size()
					: pageCount;
		}
	}
	...
	public static final QName QN_ELE_FILEGRP = new QName(METS_NAMESPACE,
			"fileGrp");
	public static final QName QN_ATTR_FILEGRP_USE = new QName(EMPTY_NAMESPACE,
			"USE");

	public static final QName QN_ELE_FILE = new QName(METS_NAMESPACE, "file");
	public static final QName QN_ATTR_FILE_SEQ = new QName(EMPTY_NAMESPACE,
			"SEQ");
	public static final QName QN_ATTR_FILE_ID = new QName(EMPTY_NAMESPACE, "ID");
	public static final QName QN_ATTR_FILE_SIZE = new QName(EMPTY_NAMESPACE,
			"SIZE");
	public static final QName QN_ATTR_FILE_CHECKSUM = new QName(
			EMPTY_NAMESPACE, "CHECKSUM");
	public static final QName QN_ATTR_FILE_CHECKSUMTYPE = new QName(
			EMPTY_NAMESPACE, "CHECKSUMTYPE");

	public static final QName QN_ELE_FLOCAT = new QName(METS_NAMESPACE,
			"FLocat");
	public static final QName QN_ATTR_FLOCAT_XLINKHREF = new QName(
			XLINK_NAMESPACE, "href");

	public static final QName QN_ELE_STRUCTMAP = new QName(METS_NAMESPACE,
			"structMap");
	public static final QName QN_ATTR_STRUCTMAP_TYPE = new QName(
			EMPTY_NAMESPACE, "TYPE");
	...
	public static final QName QN_ATTR_DIV_ORDER = new QName(EMPTY_NAMESPACE,
			"ORDER");
	public static final QName QN_ATTR_DIV_LABEL = new QName(EMPTY_NAMESPACE,
			"LABEL");
	public static final QName QN_ATTR_DIV_ORDERLABEL = new QName(
			EMPTY_NAMESPACE, "ORDERLABEL");

	public static final QName QN_ELE_FPTR = new QName(METS_NAMESPACE, "fptr");
	public static final QName QN_ATTR_FPTR_FILEID = new QName(EMPTY_NAMESPACE,
			"FILEID");

	private static Logger log = Logger.getLogger(METSParser.class);

	protected File metsFile;
	protected VolumeRecord volumeRecord;
	protected XMLInputFactory factory;

	public METSParser(File metsFile, VolumeRecord volumeRecord,
	...
	public void parse() throws FileNotFoundException, XMLStreamException,
		...
		if (log.isTraceEnabled())
			log.trace("parsing METS for volume " + volumeRecord.getVolumeID());
		...
		do {
			...
			case XMLStreamConstants.START_ELEMENT: {
				...
				if (log.isTraceEnabled())
					log.trace("qName: " + qName);
				if (phaseStack.isEmpty()) {
					if (log.isTraceEnabled())
						log.trace("**** START_ELEMENT: stack empty");
					if (QN_ELE_FILEGRP.equals(qName)) {

						HashMap<QName, String> fileGrpAttributesMap = getAttributes(reader);
						String attributeValue = fileGrpAttributesMap
								.get(QN_ATTR_FILEGRP_USE);
						if (attributeValue != null
								&& "ocr".equals(attributeValue)) {
						...
						String typeAttributeValue = structMapAttributesMap
								.get(QN_ATTR_STRUCTMAP_TYPE);
						if (typeAttributeValue != null
								&& "physical".equals(typeAttributeValue)) {
					...
					if (log.isTraceEnabled())
						log.trace("**** START_ELEMENT: stack top "
								+ phase.toString());
					...
					case OCR_FILEGRP: {
						if (QN_ELE_FILE.equals(qName)) {
							...
							String idAttributeValue = fileAttributesMap
									.get(QN_ATTR_FILE_ID);
							if (log.isTraceEnabled())
								log.trace("ID: " + idAttributeValue);

							if (idAttributeValue != null) {

								String sizeAttributeValue = fileAttributesMap
										.get(QN_ATTR_FILE_SIZE);
								String seqAttributeValue = fileAttributesMap
										.get(QN_ATTR_FILE_SEQ);
								String checksumAttributeValue = fileAttributesMap
										.get(QN_ATTR_FILE_CHECKSUM);
								String checksumTypeAttributeValue = fileAttributesMap
										.get(QN_ATTR_FILE_CHECKSUMTYPE);

								if (log.isDebugEnabled()) {
									log.debug("SIZE: " + sizeAttributeValue);
									log.debug("SEQ: " + seqAttributeValue);
									log.debug("CHECKSUM: "
											+ checksumAttributeValue);
									log.debug("CHECKSUMTYPE: "
											+ checksumTypeAttributeValue);
								}

								long size = Long.parseLong(sizeAttributeValue);

								PageRecord pageRecord = volumeRecord
										.getPageRecordByID(idAttributeValue);
								if (pageRecord == null) {
									pageRecord = new PageRecord();
									pageRecord.setByteCount(size);
									pageRecord.setChecksum(
											checksumAttributeValue,
											checksumTypeAttributeValue);
									pageRecord.setID(idAttributeValue);
									pageRecord.setSequence(seqAttributeValue);

									// volumeRecord.addPageRecord(pageRecord);
									// // cannot add yet because xlink:href is
									// not known

								} else {
									log.warn("Duplicate page ID in METS. ID: "
											+ idAttributeValue + " metsFile: "
											+ metsFile.getPath());
								}
								...
								log.warn("Null ID attribute in METS. metsFile: "
										+ metsFile.getPath());
							}
					...
					case FILE: {
						if (QN_ELE_FLOCAT.equals(qName)) {
							...
							String xlinkHrefAttributeValue = fLocatAttributesMap
									.get(QN_ATTR_FLOCAT_XLINKHREF);
							if (log.isTraceEnabled())
								log.trace("xlink:href: "
										+ xlinkHrefAttributeValue);

							if (xlinkHrefAttributeValue != null) {
								if (currentPageRecord != null) {
									currentPageRecord
											.setOcrFilename(xlinkHrefAttributeValue);
									volumeRecord
											.addPageRecord(currentPageRecord);
									phaseStack.push(ParsePhaseEnum.FLOCAT);
								} else {
									log.warn("Null currentPageRecord. xlink:href: "
											+ xlinkHrefAttributeValue
											+ " metsFile: "
											+ metsFile.getPath());
								}
								...
								log.warn("Null xlink:href. metsFile: "
										+ metsFile.getPath());
							}
					...
					case STRUCTMAP: {
						if (QN_ELE_DIV.equals(qName)) {
							...
							String divTypeAttributeValue = divAttributesMap
									.get(QN_ATTR_DIV_TYPE);
							if (divTypeAttributeValue != null
									&& "volume".equals(divTypeAttributeValue)) {
					...
					case VOLUME_DIV: {
						if (QN_ELE_DIV.equals(qName)) {
							...
							String divTypeAttributeValue = divAttributesMap
									.get(QN_ATTR_DIV_TYPE);
							if (divTypeAttributeValue != null
									&& "page".equals(divTypeAttributeValue)) {
								String orderAttrValue = divAttributesMap
										.get(QN_ATTR_DIV_ORDER);
								if (orderAttrValue != null) {
									int order = Integer
											.parseInt(orderAttrValue);

									String divLabelAttrValue = divAttributesMap
											.get(QN_ATTR_DIV_LABEL);
									String divOrderLabelAttrValue = divAttributesMap
											.get(QN_ATTR_DIV_ORDERLABEL);

									currentPageDivBean = new PageDivBean();
									currentPageDivBean.setOrder(order);
									currentPageDivBean
											.setLabel(divLabelAttrValue);
									currentPageDivBean
											.setOrderLabel(divOrderLabelAttrValue);

								} else {
									log.warn("Missing ORDER in div. metsFile: "
											+ metsFile.getPath());
								}
								...
								log.warn("Missing TYPE in div. metsFile: "
										+ metsFile.getPath());
							}
						}
					...
					case PAGE_DIV: {
						if (QN_ELE_FPTR.equals(qName)) {
							...
							String fileIDAttrValue = fptrAttributesMap
									.get(QN_ATTR_FPTR_FILEID);
							if (fileIDAttrValue != null) { // &&
															// fileIDAttrValue.matches(OCR_FILEID_REGEX))
															// {
								if (currentPageDivBean != null) {
									PageRecord pageRecord = volumeRecord
											.getPageRecordByID(fileIDAttrValue);
									if (pageRecord != null) {
										int order = currentPageDivBean
												.getOrder();
										pageRecord.setOrder(order);
										String orderLabel = currentPageDivBean
												.getOrderLabel();
										pageRecord.setLabel(orderLabel);
										String featuresAttrValue = currentPageDivBean
												.getLabel();

										if (log.isDebugEnabled()) {
											log.debug("currentPageDivBean.getOrder() "
													+ order);
											log.debug("currentPageDivBean.getOrderLabel() "
													+ orderLabel);
											log.debug("currentPageDivBean.getLabel() "
													+ featuresAttrValue);
										}

										if (featuresAttrValue != null
												&& !"".equals(featuresAttrValue
														.trim())) {
											addPageFeatures(pageRecord,
													featuresAttrValue);
										}
										phaseStack
												.push(ParsePhaseEnum.OCR_FPTR);

										// } else {
										// log.warn("Null PageRecord retrieved for FILEID: "
										// + fileIDAttrValue + " metsFile: " +
										// metsFile.getPath());
									}
								} else {
									if (log.isDebugEnabled())
										log.debug("Null currentPageDivBean for FILEID: "
												+ fileIDAttrValue
												+ " metsFile: "
												+ metsFile.getPath());
								}
			...
			case XMLStreamConstants.END_ELEMENT: {
				if (!phaseStack.isEmpty()) {
					ParsePhaseEnum phase = phaseStack.peek();
					if (log.isTraceEnabled())
						log.trace("**** END_ELEMENT: stack top "
								+ phase.toString());

					QName qName = reader.getName();
					if (log.isTraceEnabled())
						log.trace("qName: " + qName);
					if ((QN_ELE_FLOCAT.equals(qName) && ParsePhaseEnum.FLOCAT
							.equals(phase))
							|| (QN_ELE_FILE.equals(qName) && ParsePhaseEnum.FILE
									.equals(phase))
							|| (QN_ELE_FILEGRP.equals(qName) && ParsePhaseEnum.OCR_FILEGRP
									.equals(phase))) {
						phaseStack.pop();
						currentPageRecord = null;
					} else if ((QN_ELE_FPTR.equals(qName) && ParsePhaseEnum.OCR_FPTR
							.equals(phase))
							|| (QN_ELE_DIV.equals(qName) && (ParsePhaseEnum.PAGE_DIV
									.equals(phase) || ParsePhaseEnum.VOLUME_DIV
									.equals(phase)))
							|| (QN_ELE_STRUCTMAP.equals(qName) && ParsePhaseEnum.STRUCTMAP
									.equals(phase))) {
						phaseStack.pop();
	...
	private HashMap<QName, String> getAttributes(XMLStreamReader reader) {
		...
		for (int i = 0; i < attributeCount; i++) {
			...
			if (log.isTraceEnabled())
				log.trace("attributeQName: " + attributeQName
		...
		try {
			...
			if (bufferedReader != null) {
				try {
					...
					log.error(
							"Unable to close BufferedReader for "
									+ file.getPath(), e);
				}
			}
		}
	...
	public void setVolumeRecord(VolumeRecord volumeRecord) {
		this.volumeRecord = volumeRecord;
	}
